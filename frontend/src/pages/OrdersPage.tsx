...
import { Button, Chip, CircularProgress, InputAdornment, List, ListItemButton, ListItemText, Paper, TextField, Typography } from "@mui/material";
...
const STATUSES = ["CREATED", "PROCESSING", "CONFIRMED", "DECLINED"];
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div><Typography variant="h4">Orders</Typography><Typography color="text.secondary">Search your order history by order number or status.</Typography></div>
        <NewOrderButton />
      </div>
      <Paper className="mb-5 p-4">
        <TextField fullWidth label="Search orders" placeholder="For example: 42 or CONFIRMED" value={search} onChange={(event) => setSearch(event.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><Search /></InputAdornment>, endAdornment: search ? <InputAdornment position="end"><Button size="small" startIcon={<Clear />} onClick={() => setSearch("")}>Clear</Button></InputAdornment> : undefined }} />
      </Paper>
      {query.isLoading ? <div className="flex justify-center py-16"><CircularProgress aria-label="Loading orders" /></div> : query.isError ? <EmptyState title="Unable to load orders" description="Check your connection and try again." action={<Button onClick={() => query.refetch()}>Try again</Button>} /> : !query.data?.content.length ? <EmptyState icon={<ReceiptLong sx={{ fontSize: 48 }} />} title="No orders found" description="Change or clear your search and try again." /> : <Paper><List disablePadding>{query.data.content.map((order) => <ListItemButton divider key={order.id} onClick={() => setSelected(order)} aria-label={`View order ${order.id} details`}><ListItemText primary={`Order #${order.id}`} secondary={`${formatDateTime(order.createdAt)} · Total ${formatCurrency(order.total)}`} /><Chip label={order.status} size="small" /></ListItemButton>)}</List></Paper>}
